/*
wiinunchuck sample

copyright (c) Davide Gironi, 2011

Released under GPLv3.
Please refer to LICENSE file for licensing information.
*/

#include <stdlib.h>
#include <string.h>
#include <math.h>  //include libm
#include <avr/interrupt.h>
#include <avr/pgmspace.h>
#include <util/delay.h>

#define usart_BAUD_RATE 9600
#include <ucr/usart.h>

#include <wiinunchuck/wiinunchuck.h>
#include <wiinunchuck/wiinunchuck.c>

#define DEBUG_ENABLED 0 //enable or disable debug output

#define CRCCHECK_ENABLED 1 //enable or disable crc check

#if CRCCHECK_ENABLED == 1
//crc8 lookup for poly X^8 + X^7 + X^2 + 1
static const unsigned char PROGMEM crc8table[256] =
{
	
};
#endif

int main(void) {
	#if DEBUG_ENABLED == 1
	char printbuff[100];
	#endif

	//init usart
	initUSART();

	//init wiinunchuck
	wiinunchuck_init();

	//init interrupt
	sei();

	for (;;) {
		//data vars
		uint8_t buttonZ;
		uint8_t buttonC;
		int joyX;
		int joyY;
		int angleX;
		int angleY;
		int angleZ;

		//update data
		wiinunchuck_update();

		//get data
		buttonZ = wiinunchuck_getbuttonZ();
		buttonC = wiinunchuck_getbuttonC();
		joyX = wiinunchuck_getjoyX();
		joyY = wiinunchuck_getjoyY();
		angleX = wiinunchuck_getangleX();
		angleY = wiinunchuck_getangleY();
		angleZ = wiinunchuck_getangleZ();

		#if DEBUG_ENABLED == 1
		//itoa(buttonZ, printbuff, 10); usart_puts(printbuff); usart_puts(" ");
		//itoa(buttonC, printbuff, 10); usart_puts(printbuff); usart_puts(" ");
		//itoa(joyX, printbuff, 10); usart_puts(printbuff); usart_puts(" ");
		//itoa(joyY, printbuff, 10); usart_puts(printbuff); usart_puts(" ");
		//itoa(angleX, printbuff, 10); usart_puts(printbuff); usart_puts(" ");
		//itoa(angleY, printbuff, 10); usart_puts(printbuff); usart_puts(" ");
		//itoa(angleZ, printbuff, 10); usart_puts(printbuff); usart_puts(" ");

		#if WIINUNCHUCK_GETATTITUDE == 1
		double pitch = 0;
		double roll = 0;
		wiinunchuck_getpitchroll(angleX, angleY, angleZ, &pitch, &roll);
		pitch = pitch * 180 / M_PI; //radians to degree
		roll = roll * 180 / M_PI; //radians to degree
		//itoa((int)pitch, printbuff, 10); usart_puts(printbuff); usart_puts(" ");
		//itoa((int)roll, printbuff, 10); usart_puts(printbuff); usart_puts(" ");
		#endif

		usart_puts("\r\n");

		_delay_ms(100);
		#else

		#if CRCCHECK_ENABLED == 1
		unsigned char crcc = 0;
		crcc = pgm_read_word(&(crc8table[crcc ^ (unsigned char)buttonZ]));
		crcc = pgm_read_word(&(crc8table[crcc ^ (unsigned char)buttonC]));
		crcc = pgm_read_word(&(crc8table[crcc ^ (unsigned char)(joyX)]));
		crcc = pgm_read_word(&(crc8table[crcc ^ (unsigned char)(joyX>>8)]));
		crcc = pgm_read_word(&(crc8table[crcc ^ (unsigned char)(joyY)]));
		crcc = pgm_read_word(&(crc8table[crcc ^ (unsigned char)(joyY>>8)]));
		crcc = pgm_read_word(&(crc8table[crcc ^ (unsigned char)(angleX)]));
		crcc = pgm_read_word(&(crc8table[crcc ^ (unsigned char)(angleX>>8)]));
		crcc = pgm_read_word(&(crc8table[crcc ^ (unsigned char)(angleY)]));
		crcc = pgm_read_word(&(crc8table[crcc ^ (unsigned char)(angleY>>8)]));
		crcc = pgm_read_word(&(crc8table[crcc ^ (unsigned char)(angleZ)]));
		crcc = pgm_read_word(&(crc8table[crcc ^ (unsigned char)(angleZ>>8)]));
		#endif

		USART_Send(buttonZ);
		USART_Send(buttonC);
		USART_Send(joyX);
		USART_Send(joyX>>8);
		USART_Send(joyY);
		USART_Send(joyY>>8);
		USART_Send(angleX);
		USART_Send(angleX>>8);
		USART_Send(angleY);
		USART_Send(angleY>>8);
		USART_Send(angleZ);
		USART_Send(angleZ>>8);

		#if CRCCHECK_ENABLED == 1
		USART_Send(crcc);
		#endif

		//usart_putc('\n');

		#endif
	}
	
	return 0;
}
